...
class ChatManager:
    ...
    async def init(self):
        ...
        self.file_utils.set_index_functions(
            self.history_manager.add_file, self.history_manager.add_folder_structure
        )
    ...
    async def deploy_chatbot_method(
        ...
        **kwargs: Any,
        ...
        if not asyncio.iscoroutinefunction(coro_func):
            raise TypeError(
                f"Expected an async function, but got {type(coro_func).__name__}"
            )
    ...
    async def task_worker(self) -> None:
        ...
        while not self.task_queue.empty():
            ...
            logger.info(
                "Task completed in %.2f seconds. Queue size before: %d, after: %d",
                execution_time,
                queue_size_before,
                queue_size_after,
            )
    ...
    async def _handle_helper_mode(self, input: str, strip_json: bool = False) -> str:
        ...
        filtered_response = await self.deploy_chatbot_method(
    ...
    async def _handle_vision_mode(
        self, target: str, user_input: str, no_render: Optional[bool] = False
    ) -> str | None:
        """
    ...
    async def _handle_shell_mode(
        self, input: str = "", bypass: bool = False, no_render: bool = False
    ) -> str | None:
        """
        ...
        if not bypass:
            code_input = await self._handle_code_mode(
                PromptHelper.shell_helper(input), shell=True
            )
                ...
                if await self.ui.yes_no_prompt("Analyze the output?", default="Yes"):
                    ...
                    if self.client.keep_history and self.client.last_response:
                        await self.add_terminal_output(
                            input, output, self.client.last_response
                        )
    ...
    async def _handle_code_mode(
        self, input: str, shell: bool = False, no_render: bool = False
    ) -> str:
        """
    ...
    async def _handle_default_mode(
        ...
        if self.ui and not no_render:
            ...
            response = await self.deploy_chatbot_method(
                self.client._fetch_response, input
            )
